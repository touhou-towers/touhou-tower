    ...
    if ply:IsAdmin() then
      if string.len(str) == 0 then
        ...
        for k, v in pairs(player.GetAll()) do
          v:SendLua([[for k,v in pairs(ents.GetAll()) do if v:GetClass() == "stage" then v:Start() end end]])
        end
      else
      ...
      for k, v in pairs(player.GetAll()) do
        if WaitTime == 0 or WaitTime == 60 then
          ...
          v:SendLua(
            [[GTowerChat.Chat:AddText("4th of July Show Starting In ]] ..
              string.NiceTime(WaitTime) .. [[!", Color( 255, 50, 50, 255 ))]]
          )
        end
      end
    end
...
net.Receive(
  "ResetStage",
  function()
    for _, stage in pairs(ents.GetAll()) do
      if stage:GetClass() == "stage" then
