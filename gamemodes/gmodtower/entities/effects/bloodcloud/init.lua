...
function EFFECT:Init( data )
	...
	for i=0, 2 do
		self:DoParticle( emitter, Pos, Norm )
	end
...
function EFFECT:DoParticle( emitter, Pos, Norm )

	local particle = emitter:Add( "decals/flesh/blood" .. math.random( 1, 5 ), Pos )
	if particle then

		particle:SetVelocity( randomAngleAmt( Norm:Angle(), 4 ):Forward() * math.Rand( 160, 300 ) / 1.8 )
