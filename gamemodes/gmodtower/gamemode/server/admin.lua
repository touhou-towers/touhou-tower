...
function IsAdmin(steamid)
	return (GTowerAdmins and table.HasValue(GTowerAdmins, steamid)) or
		(GTowerAdminPrivileged and table.HasValue(GTowerAdminPrivileged, steamid)) or
		(GTowerSecretAdmin and table.HasValue(GTowerSecretAdmin, steamid))
end
...
concommand.Add(
	...
	function(ply, cmd, args, str)
		...
		if not util.IsValidModel(args[1]) then
...
concommand.Add(
	...
	function(ply, command, args)
		...
		if args[1] == "addent" and not hook.Call("DisableAdminCommand", GAMEMODE, args[1]) and ply:IsSuperAdmin() then
			...
			if EntTable.t.SpawnFunction then
				...
				if DropEnt.LoadRoom and type(DropEnt.LoadRoom) == "function" then
					DropEnt:LoadRoom()
				end
			end

			return
		elseif args[1] == "rement" and not hook.Call("DisableAdminCommand", GAMEMODE, args[1]) then
			local Ent = ply:GetEyeTrace().Entity

			if IsValid(Ent) and not Ent:IsPlayer() and Ent:GetClass() ~= "func_brush" then
				Ent:Remove()
			end

			return
		elseif args[1] == "physgun" and not hook.Call("DisableAdminCommand", GAMEMODE, args[1]) then
		...
		if args[1] == "slay" and not hook.Call("DisableAdminCommand", GAMEMODE, args[1]) then
			TargetPly:Kill()
			...
			for k, v in pairs(player.GetAll()) do
				...
				v:SendLua(
					[[GTowerChat.Chat:AddText("]] ..
						SanitizedName2 .. [[ has gagged ]] .. SanitizedName .. [[.", Color(150, 35, 35, 255))]]
				)
			end
		elseif args[1] == "mute" then
			...
			for k, v in pairs(player.GetAll()) do
				...
				v:SendLua(
					[[GTowerChat.Chat:AddText("]] ..
						SanitizedName2 .. [[ has muted ]] .. SanitizedName .. [[.", Color(150, 35, 35, 255))]]
				)
			end
		...
		elseif args[1] == "slap" and not hook.Call("DisableAdminCommand", GAMEMODE, args[1]) then
			TargetPly:TakeDamage(tonumber(args[3] or 5), ply, ply)

			if TargetPly:Alive() then
				TargetPly:SetVelocity(VectorRand() * 2048)
			end
		elseif args[1] == "money" and args[3] then
	...
	function(ply)
		if ply.HasResetData then
			for k, v in pairs(player.GetAll()) do
				...
				v:SendLua(
					[[GTowerChat.Chat:AddText("]] ..
						SanitizedName .. [[ has reset his data and left the tower.", Color(100, 100, 100, 255))]]
				)
			end
			return
		end
...
hook.Add(
	...
	function(ply)
		...
		if table.HasValue(GTowerAdmins, ply:SteamID()) or game.SinglePlayer() then
