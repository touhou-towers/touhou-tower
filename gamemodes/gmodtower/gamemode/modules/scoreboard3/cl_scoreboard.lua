...
function PLAYERS:Init()
	...
	self.PlayerList.Paint = function(panel, w, h)
	end
	...
	for id, name in pairs(self.TabNames) do
		...
		if string.StartWith(game.GetMap(), "gmt_build") or not table.HasValue(self.LobbyOnlyTabs, name) then
...
function PLAYERS:PerformLayout()
	...
	for _, tab in pairs(self.Tabs) do
		...
		if string.StartWith(game.GetMap(), "gmt_build") or not table.HasValue(self.LobbyOnlyTabs, tab.Name) then
			tab:SetTall(24)
	...
	self.PlayerList:SetTall(
		math.Clamp(curY, self.TabHeight, (self.PlayersDisplayed * self.Height) + (self.Padding + self.StartHeight))
	)
...
local function FilteredPlayerList(players)
	for i = #players, 1, -1 do
		...
		if players[i]:GetFriendStatus() == "blocked" or CheckBlockedCache(LocalPlayer(players[i])) then
...
function PLAYERS:GetPlayerList(tabname, count)
	...
	if tabname == "VIP" then
		...
		if not count then
			self:SetTitle("These people are awesome")
		end
	end
	...
	if tabname == "Admins" then
		for _, ply in ipairs(player.GetAll()) do
			if (ply:IsAdmin() and not ply:GetNWBool("SecretAdmin")) then
				table.insert(players, ply)
			end
		end
	end
	...
	if tabname == "Blocked" then
		for _, ply in ipairs(player.GetAll()) do
			...
			if ply:GetFriendStatus() == "blocked" or CheckBlockedCache(ply) then
				table.insert(players, ply)
			end
		end
...
function PLAYERS:Think()
	...
	if not self.NextLoadingUpdate or self.NextLoadingUpdate < CurTime() then
...
function PLAYER:PerformLayout()
	...
	local ActionVisible =
		(self.Action:GetWide() > 0) and not self.Player.IsLoading and Scoreboard.Customization.PlayerActionBoxEnabled
	...
	if self.Subtitle then
		...
		if self.NotificationIcon and self.NotificationIcon:GetMaterial() then
...
function PLAYER:Paint(w, h)
	...
	if DrawRespectBorder then
		...
		surface.SetDrawColor(
			Color(
				SinBetween(colorDark.r, color.r, RealTime() * 4),
				SinBetween(colorDark.g, color.g, RealTime() * 4),
				SinBetween(colorDark.b, color.b, RealTime() * 4),
				255
			)
		)
		...
		surface.DrawRect(
			self.Avatar.x - borderSize,
			self.Avatar.y - borderSize,
			self.Avatar:GetWide() + (borderSize * 2),
			self.Avatar:GetTall() + (borderSize * 2)
		)
	else
		-- Normal border
	...
	if Scoreboard.Customization.PlayerAvatarJazz(self.Player) then
		...
		surface.DrawRect(
			self.Avatar.x - borderSize,
			self.Avatar.y - borderSize,
			self.Avatar:GetWide() + (borderSize * 2) + 2,
			self.Avatar:GetTall() + (borderSize * 2) + 2
		)
	else
...
function PLAYER:OnMousePressed(mc)
	if mc == MOUSE_LEFT then
		self.ActionBoxOn = not self.ActionBoxOn
	end
...
function PLAYER:CanClick()
	return self:IsMouseInWindow() and self.Player ~= LocalPlayer() and
		not Scoreboard.Customization.PlayerActionBoxAlwaysShow and
		not self.Player.IsLoading and
		string.StartWith(game.GetMap(), "gmt_build")
end
...
function PLAYER:Think()
	...
	if not Scoreboard.Customization.PlayerActionBoxEnabled then
		self.Action:SetVisible(false)

		return
	end

	if not ValidPanel(self.Action) then
		return
	end
	...
	if Scoreboard.Customization.PlayerActionBoxAlwaysShow or (self.ActionBoxOn and self.Player ~= LocalPlayer()) then
...
function PLAYERINFO:OnMousePressed(mc)
	if mc == MOUSE_LEFT then
		self:GetParent().ActionBoxOn = not self:GetParent().ActionBoxOn
	end
end
...
function PLAYERINFO:Update()
	...
	if DrawRespectIcons:GetBool() and self:HasRespect() then
		if not ValidPanel(self.RespectIcon) then
	...
	if not self.Player:IsAdmin() and CheckFriendCache(self.Player) then
		self:SetupRightSubtitle()
		self.SubtitleRight:SetText("Friend")
	elseif not self.Player:IsAdmin() and CheckBlockedCache(self.Player) then
		self:SetupRightSubtitle()
		self.SubtitleRight:SetText("Blocked")
	elseif subtitleRight and subtitleRight ~= "" then
		self:SetupRightSubtitle()
		...
		if ValidPanel(self.RespectIcon) and DrawRespectIcons:GetBool() and self:HasRespect() then
...
function PLAYERINFO:PerformLayout()
	...
	if ValidPanel(self.SubtitleRight) and self.SubtitleRight:GetText() ~= "" then
...
function LABELICON:Paint(w, h)
	if self.OnMousePressed and not self.IsLabel and not self.NoDrawSelectionBackground and self:IsMouseInWindow() then
...
function LABELSCORE:UpdateText()
	if self.ValueFunc and IsValid(self:GetParent():GetPlayer()) then
