...
function QUEUE:NewSetting( control, text, convar )
...
function QUEUE:Init()
	...
	self.Title = Label( TRA'Queue_Title', self )
	...
	RequestButton:SetText( TRA'Request_Video' )
	...
	VoteSkipButton:SetText( TRA'Vote_Skip' )
	...
	FullscreenButton:SetText( TRA'Toggle_Fullscreen' )
	...
	RefreshButton:SetText( TRA'Refresh_Theater' )
	RefreshButton.DoClick = function(self)
		RunConsoleCommand( "cinema_refresh" )
	end
	self.Options:AddItem(RefreshButton)

	//local Settings = vgui.Create( "TheaterButton" )
	//Settings:SetText( "Settings" )
	//Settings.DoClick = function(self)
	//	RunConsoleCommand( "cinema_refresh" )
	//end
	//self.Options:AddItem(Settings)

		-- Volume slider
	local Volume = self:NewSetting( "TheaterNumSlider", TRA'Volume', "cinema_volume" )
	Volume:SetTooltip( TRA'Settings_VolumeTooltip' )
	...
	local HD = self:NewSetting( "TheaterCheckBoxLabel", TRA'Settings_HDLabel', "cinema_hd" )
	HD:SetTooltip( TRA'Settings_HDTooltip' )
	HD:AlignLeft( 16 )
	HD:AlignTop( 325 )
	...
	-- Hide Players
	local MuteAltTab = self:NewSetting( "TheaterCheckBoxLabel", TRA'Settings_MuteFocusLabel', "cinema_mute_nofocus" )
	MuteAltTab:SetTooltip( TRA'Settings_MuteFocusTooltip' )
	MuteAltTab:AlignLeft( 16 )
	MuteAltTab:AlignTop( 355 )
...
function QUEUE:Paint( w, h )
	...
	// Background
	surface.SetDrawColor( 33, 109, 177, 255 )
	surface.DrawRect( 0, 0, self:GetWide(), self:GetTall() )

	// Title
	surface.SetDrawColor( 17, 84, 143, 255 )
...
function QUEUE:PerformLayout()
	...
	self.VideoList:SetPos(0,68)
	self.VideoList:SetWidth(256)
	self.VideoList:SetHeight(196)
...
function VIDEOVOTE:Update()
	...
	if self.Video.Owner or LocalPlayer():IsAdmin() || LocalPlayer():IsUserGroup("GTowermod") or
