...
local MatStr = "resource/localization/%s.png"
...
function PANEL:Init()
	...
	self.Flag:SetPos(0,0)
	self.Flag:SetSize(16,11)
	self.Flag:SetMaterial( MatStr:format("en") )
...
--[[---------------------------------------------------------
	SetLanguage
-----------------------------------------------------------]]
function PANEL:SetLanguage( name, id, author )

	self:SetText( name )
	self.Flag:SetMaterial( MatStr:format( id ) )

	if author and author != "" then
		self:SetTooltip( TRA('TranslationsCredit',author) )
	end

end


derma.DefineControl( "DLanguageOption", "Menu Option Line", PANEL, "DButton" )