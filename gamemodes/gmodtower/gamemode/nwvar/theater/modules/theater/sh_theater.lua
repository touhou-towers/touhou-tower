...
function THEATER:GetRTLocation()
	return self.Id
end
...
function THEATER:IsPlaying()
	if theater.Theaters[1] then
		if theater.Theaters[1]._Video._VideoTitle == "NoVideoPlaying" then
			SetGlobalString("CurVideo", "No Video Playing" )
		else
			SetGlobalString("CurVideo", theater.Theaters[1]._Video._VideoTitle )
		end
	end
...
function THEATER:VideoDuration()
	...
	if self._Video:Duration() == 0 then return IdleScreenDuration end
...
function THEATER:Think()
	...
	if SERVER then
		...
		if LocalPlayer():GetRTLocation() != self:GetRTLocation() then return end
...
if SERVER then
	...
	*/
	...
	function THEATER:PlayDefault()
		...
		self:SetVideo( VIDEO:Init(ExtractURLData( IdleScreen, self )), false )
	...
	function THEATER:RequestVideo( ply, url, force )
		...
		vid:RequestInfo( function( success )
			...
			if string.StartWith(self:VideoTitle(),"GMod Tower: Play Today") then
				self:SkipVideo()
			end
		...
		for k, vid in pairs(self._Queue) do
			if vid.id == id then
				...
				if (vid:GetOwner() == ply) or
					...
					ply:IsAdmin() || ply:IsUserGroup("mikumod") then
