...
function Initialize()
	...
	for name, loc in pairs(locations) do
		GetByRTLocation( loc.Index, true )
	end
...
function PlayerJoin( ply, locId )
	...
	local Theater = GetByRTLocation(locId, true)
...
function PlayerLeave( ply, locId )
	...
	if !locId then
		locId = ply:GetRTLocation()
	end

	local Theater = GetByRTLocation(locId)
	if !Theater then return end
...
function RequestTheaterInfo( ply, force )
	...
	for _, Theater in pairs( GetTheaters() ) do
		...
		th = {
			Location = Theater:GetRTLocation(),
			Name = Theater:Name(),
			Pos = Theater:GetPos(),
