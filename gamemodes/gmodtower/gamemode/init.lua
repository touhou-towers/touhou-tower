...
umsg.Start = function(a, b)
	if s == true then
		...
		SQLLog(
			"error",
			"Umsg started without ending! (" ..
				startedumsg .. ") ORIGINAL TRACEBACK: " .. LastTraceBack .. "\n END ORIGINAL TRACEBACK.\n\n"
		)
	end
...
function GM:CheckPassword(steam, IP, sv_pass, cl_pass, name)
	if engine.ActiveGamemode() == "gmodtowerlobby" or engine.ActiveGamemode() == "intothechaos" then
		return
	end
	...
	if PortRemove ~= nil then
		IP = string.sub(IP, 1, PortRemove - 1)
	end
	...
	if table.HasValue(GTowerAdmins, steam) or table.HasValue(GTowerSecretAdmin, steam) or MultiUsers[IP] then
		...
		return false, "Please join gamemodes from the Lobby server. (13.82.130.177:27015)"
	end
...
local function CanUseFuckingModel(ply, model, skin)
	...
	if Model and ply:HasItemById(Model.MysqlId) then
		return true
	end
end
...
function GM:AllowModel(ply, model)
	return GTowerModels.AdminModels[model] == nil or ply:IsAdmin()
end
