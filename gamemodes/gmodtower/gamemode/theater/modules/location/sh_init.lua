...
function GetRTLocations( strMap )
...
function GetRTLocationNameByIndex( iIndex )

	local locations = GetRTLocations()
...
function GetRTLocationByIndex( iIndex, strMap )
	local locations = GetRTLocations( strMap )
...
function GetRTLocationByName( strName, strMap )
	local locations = GetRTLocations( strMap )
	if !locations then return end
...
function GetTeleportByName( strName, strMap )
	return GetTeleportBy( GetRTLocationByName, strName, strMap )
end

function GetTeleportByIndex( iIndex, strMap )
	return GetTeleportBy( GetRTLocationByIndex, iIndex, strMap )
end

// returns the index of the player's current location, or 0 if unknown
function Find( ply )
	
	local tblLoc = GetRTLocations()
...
function GetPlayersInRTLocation( iIndex )

	local players = {}

	for _, ply in pairs( player.GetAll() ) do
		if ply:GetRTLocation() == iIndex then
