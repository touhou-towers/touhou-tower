...
function GM:DisplayCustomizer( enable, remove )
	...
	for id, color in pairs( colorList ) do
		...
		p.DoClick = function(self)
			RunConsoleCommand( "minigolf_color", math.Round( color.r / 255, 3 ) .. " " .. math.Round( color.g / 255, 3 ) .. " " .. math.Round( color.b / 255, 3 ) )
			ToggleCursor( false )
		...
		p.Think = function(self)
			...
			if self.Hovered then
				...
				if LocalPlayer().GetBallColor && !RADIAL.Hovered then
