...
function PlayMusicFromID(idx)
	...
	if not IsValid(LocalPlayer()) then -- well this is awkward, lets try again
		timer.Simple(
			1,
			function()
				PlayMusicFromID(idx)
...
function PlaySoundFromID(idx, num)
	if idx == MUSIC_WAITING and num == 0 and GAMEMODE:IsPracticing() then
		StopAllMusic()
		NewSong("waiting", GetRandomSongFromID(MUSIC_WAITING), 80)
	else
		if idx == SOUNDINDEX_CLAP and num then
