...
hook.Add("GTowerPhysgunPickup", "NoFuncForFun", function( ply, ent )
	...
	return string.sub( Class, 1, 5 ) != "func_" && string.sub( Class, 1, 10 ) != "prop_door_"
end )

function GM:ShouldCollide(ent1, ent2)

	if ent1:GetClass() == "gmt_golfball" && ent2:IsPlayer() then
		return false
	end
	
	if ent1:IsPlayer() && ent2:GetClass() == "gmt_golfball" then
		return false
	end
