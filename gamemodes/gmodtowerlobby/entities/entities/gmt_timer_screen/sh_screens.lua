...
if CLIENT and IsLobby then
	...
	WebMat.Get("http://gmodtower.org/tower.html",nil,2045,1000)

	local function DrawAnnounceScreen(scr,w,h)
		WebMat.Draw("http:/gmodtower.org/tower.html",1250,700)
	...
	if not _GMT_ANNOUNCE_SCREEN then
		_GMT_ANNOUNCE_SCREEN = screen.New( DrawAnnounceScreen, Vector(928,-896,64) ):EnableInput(true):TrapMouseWheel(false):TrapMouseButtons(false):SetCull(true):AddToScene(true)
	end
	...
	_GMT_ANNOUNCE_SCREEN.Think = function( self )
		...
		local yaw = 135 --LocalPlayer():EyeAngles().y
		if yaw ~= self.lastYaw and self:GetViewerDistance() > 10000 then
	...
	_GMT_ANNOUNCE_SCREEN:SetPos( Vector(180,-917,70) )
	...
	_GMT_ANNOUNCE_SCREEN.DrawFacade = function(self, fading, depthoverride)
		...
		draw.SimpleText("The successor to GMT...", 'GTowerHUDHuge2', self.scrw/2, self.scrh/2,
