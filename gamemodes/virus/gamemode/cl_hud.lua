...
function GM:HUDPaint()
	...
	local state = game.GetWorld():GetNWFloat("State")
	local endtime = game.GetWorld():GetNWFloat("Time")

	local currentRound = game.GetWorld():GetNWFloat("Round")
	local maxRounds = game.GetWorld():GetNWFloat("MaxRounds")
	...
	if state ~= STATUS_INFECTING then
		if (state == STATUS_WAITING or state == STATUS_INTERMISSION or state == STATUS_PLAYING) then
			local spread = 64
			...
			if #player.GetAll() == 1 and currentRound == 0 then
				draw.WaveyText(
					"WAITING FOR PLAYERS",
					"GTowerHudCText",
					ScrW() / 2,
		...
		if (state == STATUS_PLAYING or state == STATUS_INTERMISSION) then
			local rankX = 0
	...
	if state ~= STATE_INTERMISSION then
		if LocalPlayer().GetActiveWeapon then
			...
			if IsValid(activeWeapon) and activeWeapon:GetClass() ~= "weapon_adrenaline" then
...
function GM:DamageNotes()
	for _, note in ipairs(DamageNotes) do
		if (note.Time + self.DamageNoteTime) < CurTime() then
			...
			if timer > self.DamageNoteTime then
				timer = self.DamageNoteTime
			end
...
function GM:DrawName(ply)
	...
	if (dist >= 800) then
		return
	end -- no need to draw anything if the player is far away
...
function GM:DrawHealth(ply)
	...
	draw.RoundedBox(4, 49, -1, 26, 101, Color(50, 50, 50, math.Clamp(self.DamageFade, 0, 150)))

	draw.RoundedBox(6, 50, 100 - percHealth, 25, percHealth, colorScale)
