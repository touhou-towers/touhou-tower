...
local function CalculateRanks()
	if NextCalcRank and NextCalcRank > CurTime() then
		return
	end
...
PlayersSort = function(a, b)
	...
	if not a.TrophyRank or not b.TrophyRank then
		return
	end
...
PlayerSubtitleText = function(ply)
	if GAMEMODE:GetState() == STATUS_WAITING then
		return ""
	end
...
PlayerNotificationIcon = function(ply)
	if (GAMEMODE:IsPlaying() or GAMEMODE:GetState() == STATUS_INTERMISSION) and ply:Frags() > 0 then
		CalculateRanks()
...
PlayerAvatarJazz = function(ply)
	if GAMEMODE:GetState() ~= STATUS_INTERMISSION then
		return false
	end
