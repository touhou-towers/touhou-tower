...
function GM:Think()
	for _, v in ipairs(player.GetAll()) do
		...
		if IsValid(Flame) and IsValid(Flame2) then
	...
	if (timeLeft == 15) then
		...
		HudMessage(HudMessages[5], 5, nil, true)
...
function GM:CalcView(ply, pos, ang, fov)
	if (ply.IsVirus and not VirusTP:GetBool()) or game.GetWorld().State == STATUS_WAITING then
		local dist = 150
...
hook.Add(
	...
	"ThirdDrawLocal",
	function()
		return (LocalPlayer().IsVirus and not VirusTP:GetBool()) or game.GetWorld().State == STATUS_WAITING
	end
)
...
local function ClientInfected(um)
	...
	if (not game.GetWorld().Started) then
...
local function ClientLateMusic(um)
	...
	if (musicType == MUSIC_WAITINGFORINFECTION) then
		...
		LocalPlayer().WaitingForInfection =
			CreateSound(LocalPlayer(), WaitingForInfectionMusic .. math.random(1, 8) .. ".mp3")
		LocalPlayer().WaitingForInfection:PlayEx(1, 100)
