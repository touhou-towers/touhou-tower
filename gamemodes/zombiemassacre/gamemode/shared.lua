	...
	hook.Add(
		...
		function(cmd)
			if not LocalPlayer():Alive() then
				return
			end -- Allow jumping to handle respawning
...
function GM:IsPlaying()
	return self:GetState() == STATE_PLAYING or self:GetState() == STATE_WARMUP or self:GetState() == STATE_UPGRADING
end

function GM:IsRoundOver()
	return self:GetState() == STATE_INTERMISSION or self:GetState() == STATE_UPGRADING
end
...
function GM:IsValidSpawn(spawn)
	...
	for _, ent in pairs(ents.GetAll()) do
		if (ent:IsPlayer() or ent:IsNPC()) and self:IsInRadius(ent, spawn, self.NoSpawnRadius) then
			return false
		end
	end
	...
	for _, ply in pairs(player.GetAll()) do
		if ply:Alive() and self:IsInRadius(ply, spawn, self.SpawnRadius) then
			return true
		end
	end
...
function GM:ShouldCollide(ent1, ent2)
	if ent1:IsNPC() and ent2:IsNPC() then
		return false
	end
