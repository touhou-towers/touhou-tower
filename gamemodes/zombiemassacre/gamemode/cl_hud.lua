...
function GM:HUDPaint()
	...
	if ConVarDisplayHUD:GetInt() == 0 then
		if LocalPlayer():Alive() and GAMEMODE:GetState() ~= STATE_UPGRADING then
	...
	if GAMEMODE:IsPlaying() or GAMEMODE:GetState() == STATE_UPGRADING then
		self:DrawHUDScores()
		self:DrawHUDHealth()
		self:DrawHUDClass()
	...
	if LocalPlayer():Alive() then
		...
		if GAMEMODE:GetState() ~= STATE_UPGRADING or GAMEMODE:GetState() ~= STATE_INTERMISSION then
			self:DrawHUDCrosshair()
			self:DrawHUDCombo()
		end
	else
...
function GM:DrawBlur()
	if ConVarDrawBlur:GetInt() == 0 then
		return
	end
...
function GM:DrawHUDScores()
	...
	if curFrags >= 10000 or curPoints >= 10000 then
...
function GM:DrawHUDCombo()
	if LocalPlayer():GetNWInt("Combo") < 1 and not LocalPlayer():GetNWInt("IsPowerCombo") then
		return
	end
...
function GM:DrawHUDWeapons()
	...
	if wep.IsMelee and not wep.IsQuickMelee and wep.Durability and wep.MaxDurability then
	...
	if wepname ~= "Glock" and not wep.IsMelee then
...
function GM:DrawHUDClass()
	...
	if LocalPlayer():GetNWInt("Combo") > 0 and not LocalPlayer():GetNWInt("IsPoweredUp") then
	...
	for k, ply in pairs(player.GetAll()) do
		if LocalPlayer() ~= ply then
			...
			if self:IsPlaying() then
				...
				if ply:GetNWString("Lives") == 0 and not ply:Alive() then
...
function GM:DrawHUDCrosshair()
	...
	if wep.IsMelee and not wep.IsQuickMelee then
...
function GM:DrawHUDCircle(val, max, x, y, size, alpha, inverse)
	if not val or not max or val <= 0 or val <= 0 then
		return
	end
	...
	surface.DrawPoly(GetCircleTable(math.ceil(percent), percent - math.floor(percent), size, size + 4, polys, 0, x, y))
end
...
function GM:DrawHUDBossHealth()
	...
	curPercent = not curPercent and 1 or math.Approach(curPercent, percent, math.abs(curPercent - percent) * 0.08)
...
function GM:DrawCursors()
	if GAMEMODE:GetState() ~= STATE_UPGRADING then
		return
	end

	if not LocalPlayer()._LastUpdate then
		LocalPlayer()._LastUpdate = 0
	end
	...
	if (LocalPlayer()._LastX ~= mX or LocalPlayer()._LastY ~= mY) and LocalPlayer()._LastUpdate < RealTime() - 0.5 then
...
function GM:PaintCursors()
	...
	for _, ply in pairs(player.GetAll()) do
		if LocalPlayer() ~= ply then
			...
			if not ply._CurX or not ply._CurY then
