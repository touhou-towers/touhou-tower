...
hook.Add(
	...
	function(target, dmginfo)
		...
		if target:IsNPC() and target:Health() < amount then
			...
			local melee = {
				...
				"weapon_zm_chainsaw",
				"weapon_zm_katana",
				"weapon_zm_medevilsword",
				"weapon_zm_melee_survivor",
				"weapon_zm_sledgehammer"
			}
...
hook.Add(
	...
	function(ply)
		...
		elseif not ply:IsBot() and #player.GetAll() >= 1 then
			PlayerNWSetup(ply)
...
function GM:PlayerShouldTakeDamage(ply, attacker)
	...
	if self:GetState() == STATE_WAITING or self:GetState() == STATE_UPGRADING then
		return false
	end
...
function GM:EntityTakeDamage(target, dmginfo)
	...
	if target:IsPlayer() and dmginfo:GetInflictor():IsPlayer() then
		return false
	end
	...
	if target:IsNPC() and target:Health() < amount then
		...
		umsg.End()
	elseif target:IsNPC() and (not self.LastDamageNote or self.LastDamageNote < CurTime()) then
		umsg.Start("HUDNotes")
		umsg.Char(1)
...
function GM:SpawnZombies()
	...
	if GetGlobalBool("ZMDayOver", false) then
		return
	end
	...
	if (math.Rand(0, 1) <= 0.05 and GetGlobalInt("Round") >= 2) then
