...
function GM:Think()
		...
		if self:GetTimeLeft() <= 0 or everyoneReady() then
			for _, ply in ipairs(player.GetAll()) do
				...
				if self.BossRound then
					...
					umsg.Short(255)
					umsg.End()
				elseif GetGlobalInt("Round") ~= 1 and not self.LostRound then
					--for k,v in pairs(player.GetAll()) do
					umsg.Start("HUDMessage", ply)
					umsg.Char(10)
					umsg.Char(8)
	...
	elseif self:GetState() == STATE_WARMUP and self:GetTimeLeft() <= 0 then
		...
		self:BeginGame()
	elseif self:GetState() == STATE_PLAYING and self:GetTimeLeft() <= 15 and self:GetTimeLeft() > 0 then
		local heli = GetGlobalEntity("Helicopter")

		if not IsValid(heli) then
			self:SpawnHelicopter()
		end
	elseif self:GetState() == STATE_PLAYING and self:GetTimeLeft() <= 0 then
		...
		self:GiveMoney()
		self:EndDay()
	elseif self:GetState() == STATE_INTERMISSION and self:GetTimeLeft() <= 0 then
		for k, v in pairs(ents.FindByClass("zm_player_gravestone")) do
			v:Remove()
		end
	...
	if self.BossRound and self.BossSpawned and not self.HasBoss() then
		self.BossRound = false
		umsg.Start("ZMPlayMusic")
		umsg.Char(5)
		umsg.Bool(true)
	...
	if deads == player.GetCount() and self:GetState() == STATE_PLAYING then
		for _, ply in ipairs(player.GetAll()) do
			ply:Freeze(true)
