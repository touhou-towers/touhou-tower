function GM:PlayerBindPress(ply, bind, pressed)
	if bind == "+menu" or bind == "invnext" or bind == "invprev" then
		if (not pressed) then
			return false
		end
	...
	if bind == "+zoom" or bind == "+duck" or bind == "+jump" then
		return true
	end
...
local function SnapToEntityBelowCursor(radius, offset, maxsnapdist)
	...
	for id, ent in pairs(ents.FindInSphere(pos, radius)) do
		if IsValid(ent) and (ent:IsNPC() or IsValidBreakable(ent)) then
			local entpos = GetEntOffsetPos(ent) or ent:GetPos()
			local dist = entpos:Distance(pos)

			if dist < radius and dist < maxsnapdist then
				enemy = ent
				position = entpos
...
function GM:CreateMove(cmd)
	...
	if cmd:KeyDown(IN_ATTACK) and GAMEMODE:IsPlaying() then
		-- Filter out melee weapons
		if LocalPlayer().GetActiveWeapon and LocalPlayer():GetActiveWeapon() and not LocalPlayer():GetActiveWeapon().IsMelee then
			-- Finally, let's find some ents
			local vec, ent = SnapToEntityBelowCursor(128, Vector(0, 0, 20), 1024)

			-- Got one, snap away!
			if IsValid(ent) and vec then
	...
	if LocalPlayer():GetNWBool("IsFocused") then
		...
		for _, ent in pairs(entities) do
			if IsValid(ent) and ent:IsNPC() and Trace(LocalPlayer(), ent) then
