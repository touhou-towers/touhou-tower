...
CLASS.PlayerTwo = nil
// DEBUG ONLY
/*local mx = CreateClientConVar( "mx", 0, true, false )
local my = CreateClientConVar( "my", 20, true, false )
local mz = CreateClientConVar( "mz", 50, true, false )
local lx = CreateClientConVar( "lx", 0, true, false )
local ly = CreateClientConVar( "ly", 0, true, false )
local lz = CreateClientConVar( "lz", 64, true, false )*/
...
function CLASS:PlayerIsClass( ply )
	return IsValid( ply ) && classmanager.IsClass( ply, self.ClassName )
end
...
function CLASS:Think()
	...
	if self:PlayerIsClass( LocalPlayer() ) || LocalPlayer():GetNWBool( "Upgraded" ) then
		self:SetCursor( "default" )
	else
		self:SetCursor( "hand" )
	end
	/*if LocalPlayer().Confirmed && self:GetOwner() != LocalPlayer() then
		self:SetAlpha( 50 )
	else
		self:SetAlpha( 255 )
	end*/
...
function CLASS:Paint( w, h )
	...
	// Border
	if ( self:IsMouseInWindow() && !self:IsFull() && !LocalPlayer():GetNWBool( "Upgraded" ) ) || self:PlayerIsClass( LocalPlayer() ) then
...
function CLASS:OnMousePressed( mc )
	if self:IsFull() || self:PlayerIsClass( LocalPlayer() ) || LocalPlayer():GetNWBool( "Upgraded" ) then return end
...
function CLASS:SetPlayer( ply )
	if !IsValid( self.PlayerOne ) then
		self.PlayerOne = ply
	...
	elseif !IsValid( self.PlayerTwo ) then
		self.PlayerTwo = ply
...
function CLASS:ClearPlayer( ply )
	if IsValid( self.PlayerOne ) && self.PlayerOne == ply then
		self.PlayerOne = nil
	...
	if IsValid( self.PlayerTwo ) && self.PlayerTwo == ply then
		self.PlayerTwo = nil
...
function CLASS:HandleSwap()
	if !IsValid( self.PlayerOne ) && IsValid( self.PlayerTwo ) then
		// Remove
...
function CLASS:HasPlayer( ply )
	return ( IsValid( self.PlayerOne ) && self.PlayerOne == ply ) || ( IsValid( self.PlayerTwo ) && self.PlayerTwo == ply )
end
...
function CONFIRM:Paint( w, h )
	...
	surface.DrawRect( 0, 0, self:GetWide(), self:GetTall() )
	// Border
	if self:IsMouseInWindow() || LocalPlayer():GetNWBool( "Upgraded" ) then
...
function Open()
	...
	LocalPlayer():SetNWBool( "Upgraded", false ) --??? -zak
	if GTowerChat && GTowerChat.Chat then
...
/*function UpdateClass( ply, class )
	for _, panel in pairs( Gui.ClassSelect.ClassPanels ) do
		local panelclass = string.lower( panel.ClassName )

		if panelclass == class then
			panel:ReassignOwner( ply )
		end
	end
end*/
...
usermessage.Hook( "ZMClassSelector", function( um )
	local hide = um:ReadBool()

	ClassSelectionWindow(hide)
end )

concommand.Add( "zm_openclass", function( ply )
	if !ply:IsAdmin() then return end
	ClassSelection.Open()
end )

concommand.Add( "zm_closeclass", function( ply )
	ClassSelection.Close()
end )
